...
public class QuestionAdapter extends FragmentStateAdapter {
    ...
    public QuestionAdapter(List<Pregunta> questionList, @NonNull FragmentActivity fragmentActivity, questionFragment mview, ViewPager2 viewPager) {
        ...
        viewPager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
            ...
            public void onPageSelected(int position) {
                    ...
                    if (position < pagerIndicatorActual) {
                        ...
                        if (questions.get(position).getTipoCampo() == 2) {
                            ...
                            if (mview.checklist.get(position).getDescAnswerAbierta().isEmpty()) {
                                ...
                                // Si viene contestada
                                if (fragment instanceof QuestionFragment2) {
                                    ((QuestionFragment2) fragment).updateView(questions.get(position));
                                }
                            }
                        } else {
                            // Cuando no es Type 2
                ...
                // Para actualizar la vista cada que se desliza
                    /*Fragment fragment = fragmentActivity.getSupportFragmentManager().findFragmentByTag("f" + position);
                    if (fragment instanceof QuestionFragment2) {
                        ((QuestionFragment2) fragment).updateView(questions.get(position));
                    }*/
    ...
    public static void updateViewPagerAtPosition() {
        if (fragmentGlobal instanceof QuestionFragment2) {
            ((QuestionFragment2) fragmentGlobal).updateView(questions.get(pos));
        }
    }
